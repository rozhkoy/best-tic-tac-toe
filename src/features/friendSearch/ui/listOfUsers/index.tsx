							...
							<React.Fragment key={pageIndex}>
								{page.rows.map(({ status, btnsStatus, nickname, userId, invitationId }, itemIndex, page) => {
									if (pageIndex === pages.length - 1 && itemIndex === page.length - 1) {
										return (
											<FriendItem ref={ref} userId={userId} key={userId + itemIndex} variant={'secondary'} src={''} status={status} nickname={nickname}>
												{children(btnsStatus, { userId, invitationId }, { page: pageIndex, item: itemIndex }, status)}
											</FriendItem>
										);
									} else {
										return (
											<FriendItem key={userId + itemIndex} userId={userId} variant={'secondary'} src={''} status={status} nickname={nickname}>
												{children(btnsStatus, { userId, invitationId }, { page: pageIndex, item: itemIndex }, status)}
											</FriendItem>
										);
									}
